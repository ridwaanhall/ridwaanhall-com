...
- **Foreground shades mirror around 500; surfaces do not.** `50↔950, 100↔900, 200↔800, 300↔700, 400↔600` preserves contrast against the canvas, which is right for text and borders. It is wrong for surfaces: in dark, `zinc-800/900/950` spread across the wide dark end of the ramp above a black canvas; in light they are compressed just under white. Those three are hand-set as a tight near-white ramp (`96.7% / 98.5% / 100%`) that keeps the elevation order canvas > 950 > 900 > 800.
- **Accents use a *different* table from zinc** (`300→800, 400→700, 500→600, 600→500, 700→300`). A plain mirror preserves distance from the end of the ramp, not perceived contrast, and the high-lightness hues are the casualty: `text-green-400` on `bg-green-900/30` measured **2.92:1** under a plain mirror versus 7.43:1 in dark. Pushing foregrounds two-to-three steps darker brings every badge to AA. If you touch this table, re-measure — this failure is invisible to every automated check the repo has.
- **Shadows are correctly *not* remapped.** Tailwind's default shadow scale hardcodes `rgba(0,0,0,·)` rather than routing through a variable, so `shadow-xl` stays dark in light mode, which is what you want — shadows are cast light, not painted surface. Only explicitly-colored ones (`shadow-black/60`, `ring-black/5`) resolve through `--color-black` and therefore vanish; those have per-selector overrides at the bottom of `input.css`.
...
- **Four stylesheets sit outside Tailwind** and carry their own `html[data-theme="light"]` blocks: `globalScrollbar.css` (which also exports the shared `--scrollbar-*` tokens the other two reuse), `sidebarSearch.css`, `imageLightbox.css`, `clickRipple.css`.
...
The compiled Tailwind output filename (currently `global-ewllzlra.css`) is a hand-picked string, not an auto-generated hash. It's hardcoded in three places that must stay in sync:
