 ...
 *
 * One copy. These were byte-identical in the guestbook's sign-in card and the
 * comment section's, and a third sign-in surface -- `/sign-in` -- is what made
 * that untenable rather than merely untidy.
 *
 * No `process.env` in here, deliberately: this module has no `"use client"` of
 * its own, so it becomes client code the moment a client component imports it,
 * and anything without a `NEXT_PUBLIC_` prefix would read as `undefined` there.
 * Pure markup is safe to be either.
 */
