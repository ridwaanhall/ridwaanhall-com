/**
 * The row of facts a card carries: when, what kind, where, how much, via whom.
 *
 * Its own module because three cards use it now -- the application card it came
 * from, education, and the openhire position card -- and it belonged to none of
 * them. Each had been spelling out its own version: different type size,
 * different gaps, different icon size, and three different glyphs for
 * "location", so cards a reader meets one after another disagreed about what a
 * fact looks like.
 */

/**
 * The gaps are wider than a plain `gap-4` because these carry no background:
...
export type FactKind = "employment" | "locationType" | "location" | "salary" | "via";
...
export function FactIcon({ kind }: { kind: FactKind }) {
